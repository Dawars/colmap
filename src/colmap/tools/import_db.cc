...
FeatureKeypointsBlob get_keypoints(
    ...
    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> keypoints;
...
std::string names_to_pair(const std::string& name0, const std::string& name1, const std::string& separator = "/") {
...
std::string names_to_pair_old(const std::string& name0, const std::string& name1) {
  return names_to_pair(name0, name1, "-");
}

// Function to find the appropriate pair
std::pair<std::string, bool> find_pair(const H5::H5File& hfile, const std::string& name0, const std::string& name1) {
  ...
  throw std::runtime_error("Could not find pair (" + name0 + ", " + name1 + ")... Maybe you matched with a different list of pairs?");
}

// Define a utility function to create an Eigen::Map from std::vector<std::pair<int, int>>
Eigen::Map<Eigen::Matrix<uint32_t, Eigen::Dynamic, 2, Eigen::RowMajor>> vector_to_eigen_map(std::vector<std::pair<int, int>>& vec) {
  // Ensure that std::pair<int, int> is laid out contiguously as expected
  static_assert(sizeof(std::pair<int, int>) == 2 * sizeof(int), "std::pair<int, int> is not laid out as expected");

  // Create an Eigen::Map that treats the vector data as an Eigen matrix
  return Eigen::Map<Eigen::Matrix<uint32_t, Eigen::Dynamic, 2, Eigen::RowMajor>>(
      reinterpret_cast<uint32_t*>(vec.data()), vec.size(), 2);
}

std::tuple<FeatureMatchesBlob, std::vector<double>> get_matches(
    const std::string& path, const std::string& name0, const std::string& name1) {
...
void import_matches(
    const std::unordered_map<std::string, int>& image_ids,
    const std::string& database_path,
  ...
  for (const auto& [name0, name1] : tq::tqdm(pairs)){
...
int main(int argc, char** argv) {
  ...
  std::string database_path, pairs_path, features_path, matches_path, image_path;

  options.AddRequiredOption("database_path", &database_path);
  options.AddRequiredOption("pairs_path", &pairs_path);
  options.AddRequiredOption("features_path", &features_path);
  options.AddRequiredOption("matches_path", &matches_path);
  options.AddRequiredOption("image_path", &image_path);
  options.AddBundleAdjustmentOptions();
  options.Parse(argc, argv);
//  std::string database_path = "/mnt/hdd/3d_recon/theater_hloc/features/database.db";
//  std::string pairs_path = "/mnt/hdd/3d_recon/theater_hloc/features/pairs.txt";
//  std::string features_path = "/mnt/hdd/3d_recon/theater_hloc/features/feats-disk.h5";
//  std::string matches_path = "/mnt/hdd/3d_recon/theater_hloc/features/feats-disk_matches-disk-lightglue_pairs.h5";
//  std::string image_path = "/mnt/hdd/datasets/own/budapest/nepszinhaz_medium/images";


  if (ExistsDir(database_path)) {
